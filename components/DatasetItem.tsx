...
export function DatasetItem({ item }: DatasetItemProps) {
  return (
    <Link href={`/item/${item.id}`}>
      <Card className="cursor-pointer hover:shadow-lg transition-shadow duration-300">
        ...
        <CardContent>
          ...
          <p className="text-sm text-muted-foreground">Size: {item.datasetSize}</p>
          <p className="text-sm text-muted-foreground">Format: {item.fileFormat}</p>
